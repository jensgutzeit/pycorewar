/* Optimizingmodule.c - Corwar.Optimizing
 *
...
/*
 * Helper functions for Corewar.Optimizing.MARS_94nop
 */
...
MARS_94nop_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	/* Get memory for information about program execution. */
...
PyDoc_STRVAR(MARS_94nop_insnfreq__doc__,
"Dictionary with the frequency of each executed instruction.");

static PyObject *
MARS_94nop_GetInsnFreq(MARS_94nop *self, void *closure)
{
	static const char *OPCODES[] = {"dat", "mov", "add", "sub", "jmp",
				        "jmz", "jmn", "djn", "seq", "spl",
				        "slt", "mul", "div", "mod", "sne",
				        "nop"};
	static const char *MODIFIERS[] = {"f ", "a ", "b ", "ab", "ba", "x ",
					  "i "};
	...
	for (i = 0; i < 8192; i++) {
		if (self->info->count[i] != 0) {
			/* Build key. */
			snprintf(key, 10, "%s.%s%s %s",
				 OPCODES[OPCODE(i)], MODIFIERS[MODIFIER(i)],
				 MODES[AMODE(i)], MODES[BMODE(i)]);

			/* Build value. */
			insn_freq =
			     PyLong_FromUnsignedLongLong(self->info->count[i]);
			PyDict_SetItemString(freq, key, insn_freq);	
		}
	}
...
MARS_94nop_run(MARS_94nop *self, PyObject *args, PyObject *kwds)
{
	...
	result = run_94nop(self->core, wlist, self->coresize,
			   ...
			   self->mindistance, rounds, seed, self->info);

	/* Build results. */
	res = Py_BuildValue("[[KKK][KKK]K]", result[0], result[1], result[2],
	...
	for (i = 0; i < numwarriors; i++) {
		...
		for (j = 0; j < numwarriors + 1; j++) {
			value = PyLong_FromUnsignedLongLong(
						result[i*(numwarriors+1)+j]);
			PyList_SetItem(tmp, j, value);
		}
	}

	/* Set number of executed instructions. */
	value = PyLong_FromUnsignedLongLong(
					result[numwarriors*(numwarriors+1)]);
	PyList_SetItem(res, numwarriors, value);
...
static PyGetSetDef MARS_94nop_getseters[] = {
	...
	{"insnfreq", (getter)MARS_94nop_GetInsnFreq, NULL,
	 MARS_94nop_insnfreq__doc__, NULL},
	{NULL} /* sentinel */
};
...
"Package for optimizing warriors\n\n"\
"Classes:\n\n"\
