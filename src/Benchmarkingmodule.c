 ...
 */
insn_t *
get_instructions(PyObject *insnList, u32_t numinsns)
{
	PyObject *insn;
	insn_t *insns;
	u32_t i;
	...
	for (i = 0; i < numinsns; i++) {
		...
		/* FIXME: Check type of insn. */
		if (insn == NULL) {
			free(insns);
			return NULL;
		}
		if (copy_instruction(insn, &insns[i])) {
			Py_DECREF(insn);
			free(insns);
			return NULL;
		}
 ...
 * 1 on error. */
u32_t
get_warrior(PyObject *w, warrior_t *warrior, u32_t maxlength)
{
	PyObject *tmp;
	s32_t length;
	s32_t start;
	...
	/* Get instructions. */
	warrior->insns = get_instructions(tmp, length);
	Py_DECREF(tmp);
...
warrior_t *
get_warriors(PyObject *warriors, u32_t numwarriors, u32_t maxlength)
{
	...
	for (i = 0; i < numwarriors; i++) {
		...
		if (get_warrior(warrior, &wlist[i], maxlength)) {
			Py_DECREF(warrior);
			for (j = 0; j < i; j++) {
				free(wlist[i].insns);
			}
...
MARS_88_run(MARS_88 *self, PyObject *args, PyObject *kwds)
{
	...
	wlist = get_warriors(warriors, 2, self->maxlength);
	if (wlist == NULL) {
	...
	if (result[4] == MARS_88_INVALID_INSN) {
		free_warriors(wlist, 2);
		free(result);
		PyErr_SetString(PyExc_ValueError,
				"Executed invalid instruction.");
		return NULL;
	}
...
MARS_88_mw_run(MARS_88 *self, PyObject *args, PyObject *kwds)
{
	...
	/* Try to convert warriors into internal format. */
	wlist = get_warriors(warriors, numwarriors, self->maxlength);
	if (wlist == NULL) {
		PyErr_SetString(PyExc_ValueError, "Couldn't load warriors.");
		return NULL;
	}
...
MARS_94nop_p_run(MARS_94nop *self, PyObject *args, PyObject *kwds)
{
	...
	/* Build results. */
	res = Py_BuildValue("[[KKK][KKK]]", result[0], result[1], result[2],
			    result[1], result[0], result[2]);
	if (res == NULL) {
		free_warriors(wlist, 2);
		free(result);
